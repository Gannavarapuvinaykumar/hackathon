...
def generate_text(course):
    prompts = {
        ...
        'resources': f"Based on the course {course}, please provide a list of external resources such as YouTube video links, Wikipedia articles, and other relevant websites that would be beneficial for students to deepen their understanding of the course material. Include a brief description of each resource."
    }
    completions = {}
    ...
    for key, prompt in prompts.items():
        response = openai.ChatCompletion.create(
            ...
            temperature=0.1,
            max_tokens=1000,  # Increase tokens for more detailed responses
        )
...
def generate_module_content(course_name, module_name):
    ...
    # Parse YouTube links from the module content
    youtube_links = []
    for line in module_content.split('\n'):
        if "YouTube:" in line:
            # Extract the link after "YouTube:"
            link = line.split("YouTube:")[1].strip()
    ...
    # Combine the module content, code snippets, and YouTube links
    combined_content = f"{module_content_html}\n{code_content_html}\n{ascii_content_html}"
    
    # Add YouTube links to the combined content
    if youtube_links:
        combined_content += "\n\n<h3>YouTube Resources</h3>\n<ul>"
        for link in youtube_links:
            combined_content += f"<li><a href='{link}' target='_blank'>{link}</a></li>"
        combined_content += "</ul>"
...
def generate_recommendations(saved_courses):
    ...
    for course in saved_courses:
        # Prompt to include course name, description, and resources
        prompt = f"Based on the course {course.course_name}, please provide a single course name at the top and a concise description for a new recommended course (less than 70 characters). Additionally, include a list of external resources (e.g., YouTube video links, Wikipedia articles, or other websites) that would help students study this recommended course."
        ...
        response = openai.ChatCompletion.create(
            ...
            temperature=0.1,
            max_tokens=500,  # Increase tokens to accommodate resources
        )
        result = response['choices'][0]['message']['content'].strip()
        
        # Split the result into course name, description, and resources
        if "Resources:" in result:
            course_info, resources = result.split("Resources:", 1)
        else:
            course_info, resources = result, "No resources provided."
        
        # Extract course name and description
        if "\n" in course_info:
            course_name, description = course_info.split("\n", 1)
        else:
            course_name, description = course_info, "No description provided."
        
        # Parse resources into a list of dictionaries
        resources_list = []
        ...
        # Add the course and resources to the recommended list
